...
def run_experiment(epochs=1, batch_size=128, device="cpu",
    ...
    ablation settings and returns a dict of results. Pass in pre-built
    train_loaders/test_loaders (from get_split_mnist_tasks) to avoid
    reloading MNIST from disk on every call when running many conditions.
    """
    set_seed(seed)
